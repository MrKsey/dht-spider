...
&& apt-get install --no-install-recommends -y ca-certificates wget git jq curl software-properties-common iproute2 miniupnpc gridsite-clients cron gpg-agent gpg \
...
&& apt-get purge -y -q --auto-remove git golang-go software-properties-common gpg gpg-agent \
&& apt-get clean \
