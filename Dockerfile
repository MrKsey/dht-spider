...
&& apt-get install --no-install-recommends -y ca-certificates wget git jq curl golang iproute2 miniupnpc gridsite-clients cron gpg-agent gpg \
...
&& apt-get purge -y -q --auto-remove git golang gpg-agent gpg \
&& apt-get clean \
