...
export TORRENT_NAME=$(urlencode $(echo $TORRENT_INFO | jq -r .name 2>/dev/null) 2>/dev/null); \
redis-cli JSON.SET $TORRENT_HASH . "$TORRENT_INFO" &>/dev/null; \
echo -e "$TORRENT_HASH\t$TORRENT_NAME"; \
[ "$ADD_MAGNET" = "true" ] &&  export MAGNET=\'\"$(echo magnet:?xt=urn:btih:$TORRENT_HASH\&dn=$TORRENT_NAME$RETRACKERS_LIST)\"\' && echo "JSON.SET $TORRENT_HASH .magnet $MAGNET" | redis-cli &>/dev/null
); done
